...
def load_dvd_pipelines(device: str):
    if DVD_MODEL_REPO:
        ...
        return (
            DVDTextToVoxelPipeline.from_pretrained(DVD_MODEL_REPO, variant="base", **common_kwargs),
            DVDTextToVoxelPipeline.from_pretrained(DVD_MODEL_REPO, variant="bsp", **common_kwargs),
        )
    return (
        DVDTextToVoxelPipeline.from_files(GEN_DVD_CONFIG, GEN_DVD_CKPT, device=device),
        DVDTextToVoxelPipeline.from_files(EDIT_DVD_CONFIG, EDIT_DVD_CKPT, device=device),
    )
...
def generate_voxels(
    ...
    mesh_path = voxel_to_mesh(voxels, session_path(req, "generated_text_voxels.glb"))
    npy_path = save_voxel_coords(voxels, session_path(req, "generated_text_voxel64_coords.npy"))
...
def generation_stage2(
    ...
    glb_path = session_path(req, "generated_text_stage2.glb")
    glb.export(glb_path)
    torch.cuda.empty_cache()
...
def run_editing(
    ...
    mesh_path = voxel_to_mesh(edited, session_path(req, "edited_text_voxels.glb"))
    npy_path = save_voxel_coords(edited, session_path(req, "edited_text_voxel64_coords.npy"))
    torch.cuda.empty_cache()
...
def editing_stage2(
    ...
    glb = postprocessing_utils.to_glb(outputs["gaussian"][0], outputs["mesh"][0])
    glb_path = session_path(req, "edited_text_stage2.glb")
    glb.export(glb_path)
    torch.cuda.empty_cache()
    return glb_path, glb_path, seed
    ...
    with gr.Tab("Generation"):
        with gr.Row():
            ...
            with gr.Column():
                ...
                gen_stage2_view = LitModel3D(label="TRELLIS Text Stage 2 GLB", exposure=5.0, height=320)
    ...
    with gr.Tab("Editing"):
        with gr.Row(equal_height=False, elem_id="editing-three-col"):
            ...
            with gr.Column(scale=1, min_width=220):
                ...
                edit_stage2_view = LitModel3D(label="Edited TRELLIS Text Stage 2 GLB", exposure=5.0, height=300)
