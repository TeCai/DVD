...
def get_cosine_schedule_with_warmup(
    ...
    """
    Modified from https://github.com/huggingface/transformers/blob/v5.0.0/src/transformers/optimization.py#L140
    Cosine LR schedule with linear warmup.

    - Warmup: LR linearly increases from 0 to base LR in `num_warmup_steps`.
    - Decay: LR cosine decays from base LR to `base_lr * min_lr_ratio` over the remaining steps.

    Args:
        optimizer: torch optimizer.
        num_warmup_steps: how many steps to warm up.
        num_training_steps: total number of training steps.
        min_lr_ratio: final_lr = base_lr * min_lr_ratio (e.g. 0.1 → decay to 10% of base LR).

    Returns:
        scheduler: a LambdaLR scheduler; call scheduler.step() **every step**.
    """
    def lr_lambda(current_step: int):
        ...
        # After warmup: cosine decay from 1 → min_lr_ratio
...
class BasicTrainer(Trainer):
    ...
    Args:
        ...
        fp16_mode (str): Mixed precision mode.
            - None: No mixed precision.
            - 'inflat_all': Manual FP16 with master params and log-scale.
            - 'amp': Automatic mixed precision (FP16 autocast + GradScaler).
            - 'bf16': Automatic mixed precision with bfloat16 (autocast bf16, no GradScaler).
    ...
    def init_models_and_more(self, **kwargs):
        ...
        if self.fp16_mode == 'amp':
            ...
            # bf16 autocast does not require GradScaler
            self.master_params = self.model_params
            self.scaler = None
        ...
        if self.lr_scheduler_config is not None:
            if self.lr_scheduler_config['name'] == 'get_cosine_schedule_with_warmup':
                self.lr_scheduler = get_cosine_schedule_with_warmup(self.optimizer, **self.lr_scheduler_config['args'])
    ...
    def load_pretrained(self, load_path):
        """
        Load model for as pretrained with only model weights.
        """
        if self.is_master:
            print(f'\nLoading pretrained checkpoint from {load_path}...', end='')
            
        model_ckpts = {}
        for name, model in self.models.items():
            model_ckpt = torch.load(read_file_dist(load_path), map_location=self.device, weights_only=True)
            # should only a single model
