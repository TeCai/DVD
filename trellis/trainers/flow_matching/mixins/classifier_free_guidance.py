...
class DiscreteClassifierFreeGuidanceMixin:
    """
    Mixin for classifier-free guidance in USDMs.
    """  
    def __init__(self, *args, p_uncond: float = 0.1, **kwargs):
        super().__init__(*args, **kwargs)
        self.p_uncond = p_uncond
    ...
    def get_cond(self, cond, neg_cond=None, **kwargs):
        ...
        if self.p_uncond > 0:
            ...
            if not isinstance(cond, dict):
                ...
                # cond = dict_foreach([cond, neg_cond], lambda x: select(x[0], x[1], mask))
                # only support 1 depth
                for k,v in cond.items():
                    cond[k] = select(v, neg_cond[k], mask)
    ...
    def get_sampler(self, **kwargs) -> samplers.USDMAncestralCfgSampler:
        """
        Get the sampler for the diffusion process.
        """
        return samplers.USDMAncestralCfgSampler(self.sigma_min, self.vocab_size)